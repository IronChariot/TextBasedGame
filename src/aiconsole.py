...
class AIConsole:
    def __init__(self, consoleui: ConsoleUI, world: World):
        ...
        The current game takes place in {world.description}.
    ...
    def process_player_input(self, input):
        self.process_character_input(input, self.player_character)
    ...
    def process_character_input(self, input, character: Character):
        ...
        response, messages = chat_completion(prompt, system_prompt=self.system_prompt, console=self.consoleui)
        ...
        if action_type in ["speech", "question", "action"]:
            self.process_action(action_type, input, character)
        elif character == self.player_character:
            ConsoleUI.write_to_console(1, "Hmm, not sure what to do with that, sorry.")
    ...
    def process_action(self, type, input, character: Character):
        if type == "speech":
            ...
            # Put the speech description into the context of every other character in the same room:
            ...
            for other_character in other_characters_in_room:
                other_character.add_to_context(speech_text)
                if other_character == self.player_character:
                    ConsoleUI.write_to_console(1, speech_text)
            ...
            if "<response>" in response and "</response>" in response:
                ...
                # If the character is the player character, also show the response in the console
                if character == self.player_character:
                    ConsoleUI.write_to_console(1, response_text)
            ...
            if "<response>" in response and "</response>" in response:
                ...
                ConsoleUI.write_to_console(2, "Error: Could not perform action.")
    ...
    def query_world_state(self, query_text):
        ...
        <create>the kitchen:the lightswitch:thing:A light switch for the kitchen light.</create>
        ...
        while True:
            ...
            if "<update>" in response:
                ...
                update = response.split("<update>")[1].split("</update>")[0]
                ...
                next_message += f"Updated {update_name} to add '{update_desc}' to private_description.\n"
            ...
            if "<create>" in response:
                ...
                create = response.split("<create>")[1].split("</create>")[0]
                ...
                next_message += f"Created {create_name} in {create_location} with description '{create_desc}'.\n"
            ...
            response, messages = chat_completion(next_message, messages, system_prompt=self.system_prompt)
